...
namespace NextUnit.HardwareContext.SystemInformation
{
	public static class SysInfo
	{
		...
		public class DetailedDiskInfo
		{
			...
			{
				return $"Drive Letter: {DeviceId}{Environment.NewLine}" +
					   $"Volume Name: {VolumeName}{Environment.NewLine}" +
					   $"File System: {FileSystem}{Environment.NewLine}" +
					   $"Size: {SizeGB} GB{Environment.NewLine}" +
					   $"Free Space: {FreeSpaceGB} GB{Environment.NewLine}" +
					   $"Model: {Model}{Environment.NewLine}" +
					   $"Interface Type: {InterfaceType}{Environment.NewLine}" +
					   $"Serial Number: {SerialNumber}{Environment.NewLine}" +
					   $"Firmware Revision: {FirmwareRevision}{Environment.NewLine}";
			}
		}

		public static string GetDetailedDiskInfoString()
		{
			var diskInfo = GetDetailedDiskInfo();
			return string.Join($"{Environment.NewLine}-----------------------------------{Environment.NewLine}", diskInfo);
		}

		public static Dictionary<string, CimInstance> GetLogicalToPhysicalDiskMapping()
		{
			var physicalDisks = GetDiskDrives();
			var logicalDisks = GetLogicalDisks();
			var diskPartitions = GetDiskPartitions();
			var mapping = new Dictionary<string, CimInstance>();

			foreach (var logicalDisk in logicalDisks)
			{
				var logicalDiskId = logicalDisk.CimInstanceProperties["DeviceID"].Value.ToString();
				var associatedPartition = diskPartitions.FirstOrDefault(p =>
					p.CimInstanceProperties["DeviceID"].Value.ToString() == logicalDiskId);

				if (associatedPartition != null)
				{
					var partitionDeviceId = associatedPartition.CimInstanceProperties["DeviceID"].Value.ToString();
					var associatedPhysicalDisk = physicalDisks.FirstOrDefault(d =>
						d.CimInstanceProperties["DeviceID"].Value.ToString() == partitionDeviceId);

					if (associatedPhysicalDisk != null)
					{
						mapping[logicalDiskId] = associatedPhysicalDisk;
					}
				}
			}
		...
		public static List<DetailedDiskInfo> GetDetailedDiskInfo()
		{
			var logicalDisks = GetLogicalDisks();
			var diskMapping = GetLogicalToPhysicalDiskMapping();
			var detailedInfo = new List<DetailedDiskInfo>();

			foreach (var logicalDisk in logicalDisks)
			{
				var deviceId = logicalDisk.CimInstanceProperties["DeviceID"].Value.ToString();
				var info = new DetailedDiskInfo
				{
					DeviceId = deviceId,
					VolumeName = logicalDisk.CimInstanceProperties["VolumeName"].Value?.ToString(),
					FileSystem = logicalDisk.CimInstanceProperties["FileSystem"].Value?.ToString(),
					SizeGB = Convert.ToInt64(logicalDisk.CimInstanceProperties["Size"].Value) / (1024 * 1024 * 1024),
					FreeSpaceGB = Convert.ToInt64(logicalDisk.CimInstanceProperties["FreeSpace"].Value) / (1024 * 1024 * 1024)
				};

				if (diskMapping.TryGetValue(deviceId, out var physicalDisk))
				{
					info.Model = physicalDisk.CimInstanceProperties["Model"].Value?.ToString();
					info.InterfaceType = physicalDisk.CimInstanceProperties["InterfaceType"].Value?.ToString();
					info.SerialNumber = physicalDisk.CimInstanceProperties["SerialNumber"].Value?.ToString();
					info.FirmwareRevision = physicalDisk.CimInstanceProperties["FirmwareRevision"].Value?.ToString();
				}
	...
	'T', 'V', 'W', 'X', 'Y', '2', '3', '4', '6', '7', '8', '9',
		...
		private static readonly object cimLock = new object();
		...
		public static List<CimInstance> QueryCIMInstances(string className, string computerName = "localhost")
		{
			lock (cimLock)
			{
				...
				var session = CimSession.Create(computerName);
				try
				{
					return session?.EnumerateInstances(cimNamespace, className)?.ToList();
				}
				finally
				{
					session?.Dispose();
